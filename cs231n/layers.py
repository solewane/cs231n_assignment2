...
def conv_forward_naive(x, w, b, conv_param):
  ...
  N, C, H, W = x.shape
  F, C, HH, WW =w.shape
  H_out = int(1 + (H + 2 * pad_num - HH) / stride_num)
  W_out = int(1 + (W + 2 * pad_num - WW) / stride_num)
      ...
      for j in range(F):
          for m in range(H_out):
              for n in range(W_out):
                  out[i,j,m,n] = np.sum(x_pad[i,:,(m*stride_num):(m*stride_num+HH),(n*stride_num):(n*stride_num+WW)] * w[j,:,:,:])+b[j]
  #
...
def max_pool_forward_naive(x, pool_param):
  ...
  for m in range(H_out):
      for n in range(W_out):
          for i in range(N):
              for j in range(C):
                  out[i, j, m, n] = x[i, j, (m*stride):(m*stride+pool_height), (n*stride):(n*stride+pool_width)].max()
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
  ...
  for m in range(H_out):
      for n in range(W_out):
          for i in range(N):
              for j in range(C):
                  idx = x[i, j, (m*stride):(m*stride+pool_height), (n*stride):(n*stride+pool_width)].argmax()
                  m_idx = (idx // pool_width) + m*stride
                  n_idx = idx % pool_width + n*stride
                  dx[i, j, m_idx, n_idx]+= dout[i, j, m, n]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
